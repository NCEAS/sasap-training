...
Both the `tidyr` and the `dplyr` packages use the pipe operator - `%>%`, which may look unfamiliar. The pipe is a powerful way to efficiently chain together operations. Say you want to both filter out rows of a dataset, and select certain columns. Instead of writing
...
Much better!
In the above example, we pass unquoted column names preceded by minus (-) signs to specify which columns we dont want.
