...
Much beter!
In the above example, we pass unquoted column names preceded by `-` signs to specify which columns we dont want.
`select()` also allows us to specify which columns we do want:
...
Let's save our changes by overwriting the `catch` variable:
...
The syntax we used above for `gather()` might be a bit confusing so let's look at a diagram:

![annotated gather code](images/gather_annotated.png)

The first two arguments to `gather()` are the names of new columns that will be created and the other arguments with `-` symbols in front of them are columns to ignore in this process.
...
If you scan through the data, you may notice the values in the `catch` column are very small (these are annual catches so they are quite small).
...
What is this extremely cryptic error telling us?
...
```{r}
catch_integers <- as.integer(catch$catch_thousands)
```
...
We can find out which values are NAs like this:

```{r}
na_values <- which(is.na(catch_integers))
na_values
```
...
```{r}
catch <- catch %>% select(-catch_thousands)
head(catch)
```
...
This is a really common way of storing auxiliary information about our data of interest (catch) but, for analylitcal purposes, we often want them in the same `data.frame`.
...
Note: `left_join` and the other join functions can often guess the shared column name but, in this case, the column names differed between the two `data.frame`s so we had to be explicit.

Now our catches have the auxiliary information from the region definitions file alongside them.
Note: `dplyr` provides a complete set of joins: inner, left, right, full, semi, anti.
...
We just ran through the various things we can do with `dplyr` and `tidyr` but if you're wondering how this migth look in a real analysis.
