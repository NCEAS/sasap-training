...
- How to join multiple `data.frame`s together usingn `dplyr`
...
With `dplyr`, we saw how we could implement the Split-Apply-Combine strategy to quickly and easily analyze tabular data.
However, sometimes our tabular data are not quite in the shape needed to apply this strategy.
This is where `tidyr` comes in.

`tidyr` is a Swiss Army knife for tidying up un-tidy data.
Untidy data can mean:

- Data in Wide format: Need to make Tall in order to do anything with it
- Information we need is split across columns or smushed into a single column
- Data contains NAs or other missing value codes we don't want

We'll work thorugh just four functions from `tidyr`:
...
`gather()` turns Wide data into Tall data, and `spread()` does just the opposite.
Let's take the Byerly et al. dataset we used above to illustrate how these two functions work together:
It turns out that the data were actually already archived in Wide format and I converted them to Tall format for simplicity's sake.
But let's take a look at the original data:

```{r}
catch <- read.csv("data/byerly_et_al_catch_wide.csv", 
                  stringsAsFactors = FALSE)
head(catch)
```

To convert `catch` to Tall format, we use `gather()`:
...
The syntax might be a bit confusing at first.
Let's look at an annotated code snippet:

![annotated gather code](images/gather_annotated.png)

With `gather()`, we tell it what the result should look like, rather than how `gather()` should get there, and `gather()` does the rest for us.
The result, in this case, will have a column `species`, and the catch values will be in a new column called `catch`.

To convert **back** to Wide format, we use `gather()`'s opposite: `spread()` using very similar arguments as we used for `gather()`:
...
**Challenge:** Split the `city` column in the following `data.frame` into `city` and `state_code` columns:
...
`united()` does just the reverse of `separate()`:
...
**Challenge:** Use `united()` on your solution above to combine the `cities_df` back to its original form with just one column, `city`:
