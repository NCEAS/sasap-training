...
It is possible to add a trend line to the plot. In this case we used a linear model, and we used *geom_smoth* with method "lm", only fitting the slope and forcing the intercept to be 0 *(formula = "y ~ 0 + x")*. 
...
*Because this is very repetitive, we only select 3 Regions to be plotted*.
