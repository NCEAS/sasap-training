...
```{r, echo = FALSE}
datatable(stocks, rownames = FALSE, caption = "Stock information")
```




Source information, including who initially requested the data and the year in which it was obtained, is shown in Table 2. THIS TABLE IS TOO LONG AND I HATE THE STRIPES. I also think I should just include source info directly in this table instead of having two tables, but will wait on overall table feedback from Bryce
...
Now let's read in the brood table. After a quick visual inspection in Excel, we can see that the header doesn’t start until the sixth row, so we should tell `read_excel` to skip the first lines: 
...
coghill_brood <- coghill_brood %>% select(-Year__1,
                         -Year__2, 
                         -Year__3, 
                         -Year__4,
                         -Year__5, 
...
```{r rename columns}
colnames(coghill_brood) <- c("BroodYear", "TotalEscapement","R1.1","R0.2","R0.3","R1.2","R2.1","R1.3","R2.2",  
...
#Create columns for stock information
coghill_brood$Stock.ID <- 139 #preassigned Stock.ID
coghill_brood$Species <- 'Sockeye'
coghill_brood$Stock <- 'Coghill'
coghill_brood$Region <- 'PWS'
...
#rename columns according to predetermined column names
colnames(coghill_brood) <- c("BroodYear", "TotalEscapement","R1.1","R0.2","R0.3","R1.2","R2.1","R1.3","R2.2",
"R1.4","R2.3","R2.4","Stock.ID","Species",'Stock',"Region","Sub.Region","UseFlag")
...
formatting_scripts <- dir("data/data_formatting_scripts/", full.names = TRUE, pattern = "*\\.[rR]")
...
```{r merge}
    ...
    brood <- do.call(bind_rows,lapply(file.path(path1,files),read.csv, stringsAsFactors = F))
...
```{r}
brood$UseFlag[which(brood$TotalRecruits/brood$TotalEscapement > 50)] <- 0
...
It is possible to add a tendency line to the plot. In this case we used a linear model, in this case we used *geom_smoth* with method "lm", only fitting the slope and forcing the intercept to be 0 *(formula = "y ~ 0 + x")*. 
p-Value for the slope is shown in each plot.

```{r, warning=FALSE}
ggplot(brood_s, aes(R1.2, R1.3)) + 
  geom_point() + 
...
This section make the same plots than before. Now instead of making them all in the same image using the option *facet_wrap*, we define a loop that goes reagion by region, fit a linear model forcing the intercept to be 0, and then create the plot.

Not that here we use the slope *p-Value* obtained with the function *lm* to change the color from blue to red, if is not significant different than 0.  
...
for (i in region)
{
  ...
  slope_p <- summary(test)[4]$coefficients[2,4]
  color <- "red"
  if(slope_p<0.01) {color <- "blue"}
  summary(test)
  p <- ggplot(temp, aes(R1.2, R1.3)) + 
    geom_point() + ylim(0, 7e6) + xlim(0, 7e6) + 
    geom_smooth(method = "lm", formula = "y ~ 0 + x", color = color, se = FALSE) +
    stat_fit_glance(method = 'lm', method.args = list(formula = "y ~ 0 + x"),
                       geom = 'text', aes(label = paste("m P-value = ", signif(..p.value.., digits = 2), 
                                                        sep = "")), label.x.npc = 'right', 
                    label.y.npc = 0.35, size = 3) +
     labs( subtitle = paste('Function y =', round(summary(test)$coefficients[1], 0), "+", round(summary(test)$coefficients[2],2), 'x'))
  print(p)
}
```
...
As an example on how to create and use function in *R*, we ask the question How the diversity of age clases have change over time for each region.
...
This function take a vector containing the number of individual in each age class and return the *Simpson* diversity index $D$. 
```{r, warning=FALSE}
...
A new column is create with the *Simpson* diversity index.
```{r, warning=FALSE}
brood_s$Simpson <- NA
for (i in 1:nrow(b))
{
...
And we then plot *Simpson* diverity index over time. We included the linear equation in each plot. This information can be easity exported as a CSV to be used in other analysis.

```{r, warning=FALSE}

ggplot(brood_s, aes(BroodYear, Simpson)) + 
  geom_point(size=0.1) + 
  geom_smooth(method = "lm", 
              formula = "y ~ x", 
              se = FALSE) + 
  stat_poly_eq(aes(label = ..eq.label..), formula = "y ~ x", 
               parse = TRUE, size=3) +
  facet_wrap(~ Region)
