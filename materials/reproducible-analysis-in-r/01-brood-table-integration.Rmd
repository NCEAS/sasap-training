...
This section make the same plots than before. Now instead of making them all in the same image using the option *facet_wrap*, we define a loop that goes region by region, fit a linear model forcing the intercept to be 0, and then create the plot.
*Because this is very repetitive, we only select 3 Regions to be ploted*.
...
The same type of plot can easily be produced for a different grouping level. For this we selected 20 stock in the brood table:


```{r, warning=FALSE}
ggplot(filter(brood_s, Stock.ID<=120), aes(R1.2/1000, R1.3/1000)) + 
  geom_point(size = 0.5) + 
  geom_smooth(method = "lm", 
              formula = "y ~ 0 + x", 
              se = FALSE) + 
