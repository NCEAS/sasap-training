...
```
Figure 2.1: Location of stocks used in this data integration. Salmonid icon by Servien (vectorized by T. Michael Keesey) [License](https://creativecommons.org/licenses/by-sa/3.0/), available at [Phylopic](http://phylopic.org/)
...
### Relationship between 1.2s and 1.3s by Region
Same than above but now using a look to look the slope and p-Values
```{r, warning=FALSE}
region <- unique(b$Region)
for (i in region)
{
  temp <- filter(b, Region == i)
  test <- lm(R1.3 ~ R1.2, data = temp)
  slope_p <- summary(test)[4]$coefficients[2,4]
  color <- "blue"
  if(slope_p<0.01) {color <- "red"}
  summary(test)
  p <- ggplot(temp, aes(R1.2, R1.3)) + 
    ...
    geom_smooth(method = "lm", 
                ...
                color = color,
                se = FALSE) +
    ylim(0, 7e6) + xlim(0, 7e6) +
      stat_fit_glance(method = 'lm',
                       method.args = list(formula = "y ~ 0 + x"),
                       geom = 'text',
                       aes(label = paste("m P-value = ", signif(..p.value.., digits = 2), sep = "")),
...
### Age Diversity
Function for the Simpson diversity index.
```{r, warning=FALSE}
...
```{r, warning=FALSE}
b$Simpson <- NA
pb <- txtProgressBar(min = 1, max = nrow(b), style = 3)

for (i in 1:nrow(b))
{
  (b$Simpson[i] <- simpson(b[i,9:30])  )
  setTxtProgressBar(pb, i)
}

```

Plot how the Return ages have changed over time

```{r, warning=FALSE}

ggplot(b, aes(BroodYear, Simpson)) + 
  geom_point(size=0.1) + 
  geom_smooth(method = "lm", 
              formula = "y ~ x", 
              se = FALSE) + 
