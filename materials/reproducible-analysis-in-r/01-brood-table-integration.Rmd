...
```{r, warning=FALSE}
region <- unique(brood$Region)
for (i in region)
{
  temp <- filter(brood, Region == i)
...
```{r, warning=FALSE}
brood$Simpson <- NA
pb <- txtProgressBar(min = 1, max = nrow(brood), style = 3)

for (i in 1:nrow(brood))
{
  (brood$Simpson[i] <- simpson(brood[i,9:30])  )
...
ggplot(brood, aes(BroodYear, Simpson)) + 
  geom_point(size=0.1) + 
  geom_smooth(method = "lm", 
              formula = "y ~ x", 
              se = FALSE) + 
