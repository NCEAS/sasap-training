...
```{r read}
b <- read_excel('/home/sjclark/Brood Tables/Original_excels/112_COGHILL_BROOD_TABLE.xls', skip = 6)
colnames(b)
```
...
```{r write file, eval = F}
write.csv(b, '/home/sjclark/Brood Tables/Reformatted/Coghill_sockeye.csv', row.names = F)
```
...
t <- sourceEntireFolder('/home/sjclark/sasap-data/data-processing/broodTables/formatting', verbose = T)
```
...
path1 <- '/home/sjclark/Brood Tables/Reformatted/'
b <- readmerge(path1)
```
